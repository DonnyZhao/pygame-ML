    ...
    def move(self, time, keys, ball, data):
        ...
        if self.rect.top > 0:
            if keys[pygame.K_UP]:
                movement = True
                data.append(
                    [ball.rect.centerx/640, ball.rect.centery/480, self.rect.centery/480, ball.speed[1]+0.5, util.DIRECTION_UPWARDS]
                )
        ...
        if self.rect.bottom < util.HEIGHT:
            if keys[pygame.K_DOWN]:
                movement = True
                data.append(
                    [ball.rect.centerx/640, ball.rect.centery/480, self.rect.centery/480, ball.speed[1]+0.5, util.DIRECTION_DOWNWARDS]
                )
                self.rect.centery += self.speed * time
        if (not movement):
            data.append(
                [ball.rect.centerx/640, ball.rect.centery/480, self.rect.centery/480, ball.speed[1]+0.5, util.DIRECTION_STILL]
            )
...
# Creates text sprite
def text(texto, posx, posy, color, size):
    font = pygame.font.Font(util.get_font("normal"), size)
